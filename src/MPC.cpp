...
// The reference velocity is set to 100mph.
double ref_v = 100;
...
public:
    ...
    void operator()(ADvector& fg, const ADvector& vars) {
        ...
        double cte_factor = 10.0;
        double epsi_factor = 1.0;
        double v_factor = 0.001;
        ...
        double min_delta_gap_factor = 500.0;
        double min_a_gap_factor = 0.0001;
        ...
        for (size_t t = 0; t < N - 2; t++) {
            fg[0] += min_delta_gap_factor * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
            fg[0] += min_a_gap_factor * CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
        }
        ...
        for (size_t t = 1; t < N; t++) {
            ...
            //from video walkthrough, better fitting
...
vector<double> MPC::Solve(Eigen::VectorXd x0, Eigen::VectorXd coeffs) {
    ...
    // creates a 2 element double sector.

    //std::cout << solution.x.size() << endl;
